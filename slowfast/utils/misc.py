...
def log_model_info(model, cfg, logger,use_train_input=True):
    """
    ...
    logger.info(pp.pformat(cfg))
    logger.info("------------------------------------")
    logger.info("Model:\n{}".format(model))
    params = params_count(model)
    logger.info("Params: {:,} M".format(params/1024**2))
    tunable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    logger.info("Tuanable Params: {:,} M".format(tunable_params/1024**2))
    Tunable_Params = []
    for name, param in model.named_parameters():
        if param.requires_grad:  
            logger.info(f"Tunable Parameter Name: {name}")
    #         Tunable_Params.append(name)
    # logger.info(f"Tunable Parameter Name: {Tunable_Params}")
